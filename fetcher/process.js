...
import raw_data from "./all_raw.json" assert { type: "json" };
const { JSDOM } = jsdom;

const OUTPUT = "src/data/26.json";
...
const formatTitle = (str) => {
  if (
    [
      ...
      "กระทู้ถามเฉพาะ",
      "การขอเปลี่ยนระเบียบวาระ",
      "แจ้งให้ที่ประชุมรับทราบ",
    ].includes(str)
  )
...
const process = () => {
  ...
  for (let entry in raw_data) {
    ...
    const no = formatString(
      document.querySelector("table > tbody > tr:nth-child(3)").textContent
    );
    ...
    const score_summary_docs = [
      ...document.querySelectorAll("#mydetail_o > td > ul > li"),
    ].map((el) => {
  ...
  fs.writeFileSync(OUTPUT, formatted_data);
};
process();
